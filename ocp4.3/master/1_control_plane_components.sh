...
output=$(oc --kubeconfig "$KUBE_CONFIG_PATH" get configmap config -n openshift-kube-apiserver -ojson | \
  ...
  jq '.auditConfig.policyConfiguration.rules[]' | grep 'system:unauthenticated')
...
certificate=$(oc --kubeconfig "$KUBE_CONFIG_PATH" -n openshift-kube-apiserver get cm kube-apiserver-cert-syncer-kubeconfig -o yaml | grep 'client-certificate' | sed 's/.*client-certificate: //g')
key=$(oc --kubeconfig "$KUBE_CONFIG_PATH" -n openshift-kube-apiserver get cm kube-apiserver-cert-syncer-kubeconfig -o yaml | grep 'client-key' | sed 's/.*client-certificate: //g')
...
output=$(oc --kubeconfig "$KUBE_CONFIG_PATH" -n openshift-kube-apiserver get cm config -o yaml | grep '"bindAddress":"0.0.0.0:6443"')
if [ -z "$output" ]; then
  warn "$check_1_3_7"
else
  pass "$check_1_3_7"
